...
from modeling.Callbacks import KWorstPredictionsLogger, LogMetrics, GradientNormLogger
from modeling.Augment import AugmentedImageFolder, DefaultsTransformation
from modeling.ModelInitializer import SimpleCNNInitializer, SqueezeNetInitializer

def k_fold_cross_validation(k=4):
    with wandb.init(tags=["15_epochs"]):
        config = wandb.config
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kf.split(indices)):
            ...
            fold_metrics = metrics_logger.get_metrics()
...
def train_best_model(config):
    ...
    initializer = SimpleCNNInitializer(config) if config['initializer'] == "simple_cnn" else SqueezeNetInitializer(config)
    model = CNN(config, initializer, fold_idx=0)
    wandb_logger = WandbLogger(project='Ad_Classification')
    wandb_logger.watch(model, log="all")
